...
if(intval($idcotizacion)<1){
    $idcotizacion=0;
}
if (!empty($id) and !empty($cantidad) and !empty($precio_venta)) {
    $c1="select * from tbdetallecot,productos where id_producto=idproducto and idproducto=".$id." and idcotizacion=".$idcotizacion;
    $comprobar = mysqli_query($conexion, $c1);
    if ($row = mysqli_fetch_array($comprobar)) {
        $cant = $row['cantidad_tmp'] + $cantidad;
        $sql          = "UPDATE tbdetallecot SET cantidad='" . $cant . "', precio='" . $precio_venta . "' WHERE idproducto='" . $id . "'  and idcotizacion=".$idcotizacion; 
        ...
        $insert_tmp = mysqli_query($conexion, "INSERT INTO tbdetallecot (idproducto,cantidad,precio,idcotizacion) VALUES 
        ('$id','$cantidad','$precio_venta','$idcotizacion')");
        echo "<script> $.Notification.notify('success','bottom center','NOTIFICACIÓN', 'PRODUCTO AGREGADO A LA1 FACTURA CORRECTAMENTE')</script>";
    }
}
if (isset($_GET['id']))
{
    $id_tmp = intval($_GET['id']);
    $sql="DELETE FROM tbdetallecot WHERE iddetalle='" . $id_tmp . "' and idcotizacion=".$idcotizacion;
