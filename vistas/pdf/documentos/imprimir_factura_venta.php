...
<style type="text/css">
    ...
    table .page_footer 
    {
        width: 100%; border: none; background-color: white; padding: 2mm;
...
$simbolo_moneda = "Q";
$sql_factura    = mysqli_query($conexion, "select * from facturas_ventas where  id_factura='" . $last_factura . "'");
$count          = mysqli_num_rows($sql_factura);
if ($count == 1) {
    $rw_factura        = mysqli_fetch_array($sql_factura);
    $nombre_cliente    = $rw_factura['factura_nombre_cliente'];
    $direccion_cliente = $rw_factura['factura_direccion_cliente'];
    $telefono_cliente  = $rw_factura['factura_numero_cliente'];
    $email_cliente     = $rw_factura['correos'];
...
<?php include "encabezado_factura.php";?>
    ...
    <table cellspacing="0" style="width: 50%; text-align: left; font-size: 11pt; border: 1px solid #0A122A;-moz-border-radius: 13px;-webkit-border-radius: 12px;padding: 10px;">
        <tr>
            <td style="width:50%;" class='midnight-blue'>COMPROBANTE A</td>
        </tr>
        <tr>
            <td style="width:50%;" >
                <?php
